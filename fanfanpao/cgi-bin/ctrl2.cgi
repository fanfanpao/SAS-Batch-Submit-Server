...
#		$usrpwdf{$usrnamef} = $usrpwdf;
   ...
   if ($usrid eq "" or $usrpwdf{$usrid} ne $usrkey) 
   {
	...
	print "<a href=\"main2.cgi\">You must logon from MySASCloud main page</a>\n";
   ...
   else
   {
	...
	s/\//\/<wbr>/g;
      ...
      print "       <tr bgColor=#99bcd8><td width=100% colspan=2><font color=\"white\">/home$_/<wbr>$FORM{\"task\"}</font></td></tr>\n";
...
#	  if ($usrtypef{$usrid} == 0 or $usrtypef{$usrid} == 1) {	  
		print "	  	<a href=\"view2.cgi?curdir=$FORM{\"curdir\"}&task=$FORM{\"task\"}\"><img src=\"image/gtk_edit.png\" border=0></a> \n";	  
		print "	  	<a href=\"file2.cgi?curdir=$FORM{\"curdir\"}&task=$FORM{\"task\"}&action=delfile\"><img src=\"image/trash.png\" border=0></a></td></tr>\n";
	  }
	  
	my($curdir) = $FORM{"curdir"};
	my($taskdir) = "home/$curdir";

    $task = $FORM{"task"};

	print "<tr><td colspan=2><form method=POST action=\"exec2.cgi\">\n";
	print "<input type=\"hidden\" name=\"curdir\" value=\"$curdir\">\n";
#	print "<input type=\"hidden\" name=\"program\" value=\"/home$curdir/$task\">\n";
...
#	print "<br><h3>User Input:</h3>\n";
	print "<br>\n";
	  ...
	  print "<input type=\"reset\" name=\"\" value=\"reset\" size=\"\"> <P>\n";
		...
		elsif (-e "home$curdir/\.attr\.$task") {
			
			open(SHA, "home$curdir/\.attr\.$task") || die $!;
