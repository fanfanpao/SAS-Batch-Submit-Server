...
sub main_page {
   ...
   if ($usrid ne "" && $usrpwdf{$usrid} eq $usrkey) 
   {
		...
		print "<a href=\"main2.cgi\">You have an account and already logon.</a>\n";
   } 
   else
   {
	...
	if ($task eq "newuser") {
		...
		print "<tr><td colspan=2><form method=POST action=\"rgst2.cgi\">\n";
		...
		if ($status == 0) {
		   ...
		   if (-e "pswd2.flg" && -M "pswd2.flg" < 0.0000002)
			{ 
				...
				print UPW "$user:$pass:1:$email:home/Users/$user:\n";
				close(UPW);		

				mkdir("home/Users/$user") || die $!;	

				#copy("welcome.msg","home/Users/$user/welcome.msg") || die $!;		
				#copy("first.sas","home/Users/$user/first.sas") || die $!;					
				#copy("HelloWorld.r","home/Users/$user/HelloWorld.r") || die $!;	
				#copy("input.par","home/Users/$user/input.par") || die $!;						
				
				unlink("pswd2.flg") || die $!;
				
				print "<tr><td>success!<p>username: $user<br>password: $pass<br>useremail: $email<br></td></tr>\n";
				print "<tr><td>now go to <a href=\"http://localhost:8000/cgi-bin/main2.cgi\">main page</a> to logon for the first time.</td></tr>\n";			
			}
		}
